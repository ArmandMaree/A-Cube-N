...
\begin{document}	
	...
	\section{Background}
		...
		\subsection{The Client's Problem}
			\paragraph\indent
			The client wants to have a way of displaying all relevant information in one place in the form of a mind map. This will help to organize general life events such as meetings and. This information will come from different sources and enables the user to see all relevant information without having to go to each source individually. A key aspect is that Mind Map PIM attempts to do is to dig through the clutter contained in other PIMs and only display things the user might want to see.
		
	\section{Important Terminology}
		\begin{itemize}
			\item \textbf{Bubbles} Commercial name for the application.
	...
	\section{Architecture Requirements}
		\subsection{Access Channel Requirements}
            ...
            \subsubsection{System Access Channels}
                ...
                A technology known as RabbitMQ will be used to allow communication between our multiple back end applications such as the polling application, business logic application and the database application.
		...
		\subsection{Quality Requirements}
            ...
            \subsubsection{Reliability}
            	\paragraph\indent
                The system needs to be reliable, since it has to provide information in real time. This cannot happen if the system is constantly down. Thus hot swapping and hot reloading will be used to update the system without shutting down the service.
            ...
            \subsubsection{Scalability}
                \paragraph\indent
                This system needs to be highly scalable. The large number of users should not affect the system. Thus a queue messaging service (RabbitMQ) will be used to handle large number of requests. This will pervent backend processes responsible for data processing to be flooded with requests.
                
            \subsubsection{Flexibility}
            	\paragraph\indent
                The system must be flexible in terms of plugablity of polling services. A new polling service for a new PIM should be able to integrate with minor to no no change of the rest of the system. The system also should be able to function with any combination of PIMs that a user selects.
            ...
            \subsubsection{Auditability/Monitorability}
            	\paragraph\indent
                Aspect orientated programming will be used to maintain a log of the internal functioning of the system. Any requests made by users will thus also be recorded.
	...
	\section{Architecture Design}	
		\subsection{Infrastructure}
			...
			\begin{figure}[!h]
				\includegraphics[width=\linewidth]{MicroServices.jpg}
		...
		\subsection{Services}			
			\subsubsection{Data Polling Service}\label{DataPollingSection}
			\paragraph\indent
			Figure \ref{PollingServiceModelFigure} shows the class diagram of the pollers. A poller will either use long polling or polling with a sleep period afterwards (depending on what the API allows) to request new data from a specific PIM. Once it receives this information it will parse this information into a RawData object and add it into a queue which will be further processed by worker threads (see section  \ref{DataProcessingServiceSection}).			
						
			\begin{figure}[!h]
			\includegraphics[width=\linewidth]{PollingServiceModel.jpg}
			\caption{Data polling service.}
			\label{PollingServiceModelFigure}
			\end{figure}
			
						
			\subsubsection{Data Processing Service}\label{DataProcessingServiceSection}
			\paragraph\indent
			Figure \ref{DataProcessingModelFigure} shows the class diagram of the processing of new data. Once raw data has been added to a queue by the poller in section \ref{DataPollingSection}, one of several worker threads will dequeue this raw data and pass the data to a Natural Language Processor (NLP). Once the NLP has finished processing the data the worker thread will parse this data into a ProcessedData object which will be sent to the database for persistence (see section \ref{DatabaseServiceSection}).
			
			\begin{figure}[!h]
			\includegraphics[width=\linewidth]{DataProcessingServiceModel.jpg}
			\caption{Data processing service.}
			\label{DataProcessingModelFigure}
			\end{figure}
						
			\subsubsection{Business Service} \label{BusinessServiceSection}
			This service will start and stop polling services as users sign up/deactivate their accounts. Its purpose is basically to manage the other services based on user actions.
	...
	\section{Database and Persistence}
		\paragraph\indent
		We plan on using MongoDB as our database. This choice was made on the basis that a NoSQL database's document structure would fit our application's needs better than a standard relational database. MongoDB is also the perfect tool to use when data mining and high write loads are bought into play.
	...
	\section{Functional Requirements}		
		...
		\subsection{Use Case/Service Contracts}
			\subsubsection{Sign up}
    			...
    			\begin{figure}[!h]
    			\includegraphics[width=\linewidth]{functionalRequirementSignUp.jpg}
    		...
    		\subsubsection{Login to PIMs}
    			...
    			\begin{figure}[!h]
    			\includegraphics[width=\linewidth]{functionalRequirementLogin.jpg}
    		...
    		\subsubsection{Expand Bubble}
			    ...
			    \begin{figure}[!h]
    			\includegraphics[width=\linewidth]{functionalRequirementExpandBubble.jpg}
			...
			\begin{figure}[!h]
    			\includegraphics[width=\linewidth]{functionalRequirementDisplayData.jpg}
    		...
    		\subsubsection{Remove Bubble}
    			...
    			\begin{figure}[!h]
    			\includegraphics[width=\linewidth]{functionalRequirementRemoveBubble.jpg}
    		...
    		\subsubsection{Specify the default depth factor}
    			...
    			\begin{figure}[!h]
    			\includegraphics[width=\linewidth]{functionalRequirementSetDepth.jpg}
    		...
    		\subsubsection{Specify the default branch factor}
    			...
    			\begin{figure}[!h]
    			\includegraphics[width=\linewidth]{functionalRequirementSetBranchFactor.jpg}
		...
		\subsection{Domain Model}
			\begin{figure}[!h]
				\includegraphics[width=\linewidth]{Model-updated.jpg}
